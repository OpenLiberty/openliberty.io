...
There's a new endpoint control MBean so that you can now query and control the state of both HTTP endpoints and messaging endpoints (that is, activation specs). You can now also control the state of message endpoints using the existing `server` command.  And you can configure whether the messaging endpoint starts automatically with the associated application.
