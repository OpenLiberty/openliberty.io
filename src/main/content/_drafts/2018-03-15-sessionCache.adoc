---
layout: post
title:  "JCache Session Persistence"
date:   2018-03-19 12:05:00 +0100
categories: news
blog_description: "Introducing distributed in-memory HTTP session caching with OpenLiberty"
...
The `sessionCache-1.0` feature does not include a JCache implementation, so you need to pick one and reference it as a `<library>` in your server.xml.  For development, we have mainly been testing with Hazelcast, but here are some JCache implementations that could be used:
...
1. *Client-Server model:* A Liberty server can act as the JCache client and connect to a dedicated JCache server
2. *Peer-to-Peer model:* A Liberty server could connect with other Liberty servers that are also running with the `sessionCache-1.0` feature and configured to be part of the same cluster.
...
Each JCache implementation has its own way of configuring cluster instances. For Hazelcast, a simple configuration file might look like the following.  Note that it is not necessary to specify any Hazelcast configuration file if the default hazelcast configuration is acceptable.
...
Please consult the documentation for the implementation you select. So far we have been testing with https://mvnrepository.com/artifact/com.hazelcast/hazelcast/3.9.2[Hazelcast Core] which has some nice auto-discovery capabilities out of the box.
