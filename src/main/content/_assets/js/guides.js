...
$(document).ready(function () {
  ...
  function addPadding() {
    var lastSection = $(
      "#guides_container .category_section:last-child .guide_subcategory_section:last-child"
    );
    if (isDesktopView()) {
  ...
  $(window).on("resize", function () {
    ...
    if ($("#guide_search_input").is(":focus")) {
      ...
      if (popover_width > search_box_width) {
        $(".popover").css(
          "left",
          popover_left_position - (popover_width - search_box_width) / 2
        );
      }
    ...
    addPadding();
    var navBannerBottom =
      $("#guides_information_container").outerHeight(true) +
    ...
    if (width <= mobileBreakpoint && $(this).outerWidth() > mobileBreakpoint) {
      ...
      if ($(this).outerWidth() <= tabletBreakpoint) {
        ...
        if ($(window).scrollTop() > navBannerBottom) {
          $("#tablet_toc_accordion_container").css({
            position: "fixed",
            top: "0px",
          });
          $("#toc_column").css({
    ...
    if (width > mobileBreakpoint && $(this).outerWidth() <= mobileBreakpoint) {
      ...
      $("#toc_container ul")
        .find('img[src$="guides_gray_minus.svg"]')
        .each(function () {
          expanded_id = $(this)
            .parent()
            .attr("href")
      ...
      $("#toc_container ul")
        .find('img[src$="guides_caret_down.svg"]')
        .each(function () {
          var showSection = $(this)
            .parent()
            .parent()
            .nextUntil(".toc_separator");
          showSection.hide();
    ...
    if (
      ...
      $(this).outerWidth() <= tabletBreakpoint &&
      $(this).outerWidth() > mobileBreakpoint
    ) {
      // if scrolled past banner, show hamburger menu
      if ($(window).scrollTop() > navBannerBottom) {
        $("#tablet_toc_accordion_container").css({
      ...
      if (
        $("#tablet_toc_accordion_container").css("position") === "fixed" &&
        $("#nav_bar").css("position") === "fixed"
      ) {
        $("#tablet_toc_accordion_container").css(
          "top",
          $("#tablet_toc_accordion_container").outerHeight()
        );
      }
    }
  ...
  function accessContentsFromHash(hash, callback) {
    ...
    $("#toc_container").height(
      $("footer").offset().top + 25 - $("#toc_container").offset().top
    );
  }
  ...
  $(document).on("click", "#toc_container li > a", function (e) {
    ...
    if (isMobileView()) {
      ...
      programScrolling = true;
      $("html, body").animate(
        { scrollTop: $($(this).attr("href")).offset().top - accordionHeight },
        500,
        function () {
          programScrolling = false;
        }
  ...
  function getCategories(callback) {
    $.getJSON(
      "../../guides/guides-common/guide_categories.json",
      function (data) {
        $.each(data, function (index, category) {
          ...
          $("#toc_column > #toc_container > ul").append(
            '<div class="toc_title_container"><h1 class="toc_title">' +
              guide_sidebar_category_name +
              '</h1><p id="' +
              categoryId +
              '_num_guides" class="num_guides"></p><button class="caret_button"><img src="/img/guides_caret_up.svg" alt="Collapse" aria-label="Collapse"></button></div>'
          );
          // create div and header in guides section for each category
          $("#guides_container").append(
            '<div id="' +
              categoryId +
              '_category" class="category_section"><h3 class="guide_category_title">' +
          ...
          $.each(category.subcategories, function (j, subcategory) {
            ...
            $("#" + categoryId + "_category").append(
              '<div id="' +
                subcategoryId +
                '_section" class="guide_subcategory_section"><h4 id="' +
                subcategoryId +
                '" class="guide_subcategory_title">' +
                sub_category_name +
                '</h4><div class="row guide_subcategory_row" id="' +
                subcategoryId +
                '_row"></div></div>'
            );
  ...
  function filterGuides(key, search_value) {
    $(".guide_item").each(function (index, element) {
      ...
      for (var i = 0; i < tokens.length; i++) {
        var word = tokens[i];
        if (
          (key & title_key && title.indexOf(word) != -1) ||
  ...
  //collapse categories on mobile when filter is cleared
  function collapseAllCategories() {
    $("#toc_container ul li a").each(function () {
  ...
  function processSearch(inputValue) {
    if (inputValue.length == 0) {
      ...
      if (inputValue.indexOf("tag:") === 0) {
        ...
        filterGuides(
          title_key | description_key | tags_key | search_term_key,
          inputValue
        );
      }
  ...
  function _processSearchText(value) {
    ...
    if (value.startsWith("tag:")) {
      ...
      search_value =
        "?search=" + encodeURIComponent(searchTextWithoutTag) + "&key=tag";
