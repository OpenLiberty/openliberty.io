...
var starter_domain = isStagingSite()
  ? "https://staging-starter.mybluemix.net"
...
var allowed_builds = {
  ...
  runtime_betas: function (version) {
    return ["jakartaee9.zip", version + ".zip"];
  },
...
function getAllowedBuilds(build_type, package_locations, liberty_version) {
  return package_locations.filter(function (x) {
    ...
    var allowList = liberty_version
      ? allowed_builds[build_type](liberty_version)
  ...
  builds.forEach(function (build) {
    if (parent.hasClass("release_table_body")) {
      ...
      if (parent.parent().data("builds-id") == "runtime_releases") {
        ...
        var package_signature_locations =
          build.package_signature_locations || [];
        var sorted_package_signature_locations;
        if (
          package_signature_locations !== null &&
          package_signature_locations !== undefined
        ) {
          sorted_package_signature_locations = sortRuntimeLocations(
        ...
        if (package_locations !== null && package_locations !== undefined) {
          ...
          package_locations = getAllowedBuilds(
            "runtime_releases",
            package_locations
          );
          ...
          for (var k = 0; k < package_locations.length; k++) {
            ...
            var package_name = package_locations[k].split("=")[0].toLowerCase();
            ...
            var buildVersionYear = parseInt(
              build.version.substring(0, build.version.indexOf(".")),
              10
            );
            var buildVersionMonth = parseInt(
              build.version.substring(build.version.lastIndexOf(".") + 1),
              10
            );
            if (package_name.indexOf("jakartaee9") > -1) {
              // 21.0.0.12 and higher should be labled "Jakarta EE 9"
              package_column =
                "<td headers='" + tableID + "_package'>Jakarta EE 9</td>";
              ...
              if (
                buildVersionYear > 19 ||
                (buildVersionYear === 19 && buildVersionMonth > 5)
              ) {
                package_column =
                  "<td headers='" + tableID + "_package'>Jakarta EE 8</td>";
              } else {
                package_column =
                  "<td headers='" + tableID + "_package'>Java EE 8</td>";
              }
            } else if (package_name.indexOf("webprofile9") > -1) {
              package_column =
                "<td headers='" + tableID + "_package'>Web Profile 9</td>";
            } else if (package_name.indexOf("webprofile") > -1) {
              package_column =
                "<td headers='" + tableID + "_package'>Web Profile 8</td>";
            } else if (package_name.indexOf("microprofile3") > -1) {
              package_column =
                '<td headers="' + tableID + '_package">MicroProfile 3</td>';
            } else if (package_name.indexOf("microprofile4") > -1) {
              package_column =
                '<td headers="' + tableID + '_package">MicroProfile 4</td>';
            } else if (package_name.indexOf("microprofile5") > -1) {
              package_column =
                '<td headers="' + tableID + '_package">MicroProfile 5</td>';
            } else if (package_name.indexOf("kernel") > -1) {
              package_column =
                '<td headers="' + tableID + '_package">Kernel</td>';
            } else {
              package_column =
                '<td headers="' + tableID + '_package">All GA Features</td>';
            }
      ...
      // beta releases table only
        ...
        if (
          beta_package_locations !== null &&
          beta_package_locations !== undefined
        ) {
          sorted_beta_package_locations = sortBetaLocations(
        ...
        if (
          beta_package_sig_locs !== null &&
          beta_package_sig_locs !== undefined
        ) {
          sorted_beta_package_sig_locs = sortBetaLocations(
        ...
        if (
          ...
          // Assume the .sig files from the allowed builds are also allowed
          // without checking the .sig files.
          beta_package_locations = getAllowedBuilds(
            "runtime_betas",
            beta_package_locations,
            version
          );
          ...
          for (var d = 0; d < beta_package_locations.length; d++) {
            ...
            var beta_sig_name = beta_package_name.substring(
              0,
              beta_package_name.lastIndexOf(".")
            );
            var beta_sig_href = getURLForSig(
              beta_package_sig_locs,
              beta_sig_name + ".sig"
            );
            //========== Get URL for the .sha2 file
            ...
            if (beta_package_name.indexOf("jakarta") > -1) {
              ...
              package_column =
                "<td headers='" + tableID + "_package'>All Beta Features</td>";
            }
      ...
      // eclipse developer tools releases only
        ...
        var row = $("<tr></tr>");
        var version_column = $(
          '<td headers="' + tableID + '_version">' + build.version + "</td>"
        );
        var download_column = $(
          '<td headers="' +
            tableID +
...
function highlightAlternateRows() {
  $("#runtime_releases_table_container .release_table_body tr")
    .filter(function () {
      return (
        $(this).children().length ==
        document.getElementById("runtime_releases_table").rows[0].cells.length
      );
    })
    .filter(":even")
    .addClass("highlight_alternate_rows");

  $("#runtime_betas_table_container .release_table_body tr")
    .filter(function () {
      return (
        $(this).children().length ==
        document.getElementById("runtime_betas_table").rows[0].cells.length
      );
    })
    .filter(":even")
    .addClass("highlight_alternate_rows");
...
function sortBetaLocations(package_locations_param) {
  var sortArr = [];
  package_locations_param.forEach((x) => {
    ver = x.split("=")[0];
...
function sortRuntimeLocations(package_locations_param) {
  ...
  var newLocations = [];
  var packageNames = package_locations_param.map((x) => x.split(".")[0]);
  for (var k = 0; k < priority_array.length; k++) {
    ind = packageNames.indexOf(priority_array[k]);
    if (ind > -1) {
...
function add_invalid_message(field_id, valid) {
  if (!valid) {
    if (
      ...
      $(".starter_field[data-starter-field='" + field_id + "']").append(div);
    } else {
...
function validate_starter_inputs(event) {
  ...
  if (event && event.target.id === "Starter_Jakarta_Version") {
    for (var starter_key in starter_dependencies) {
      ...
      var EEVersionValue = $(
        ".starter_field[data-starter-field='" + starter_key + "'] select"
      )
        .find(":selected")
        .text();
      ...
      for (var d in dependencies) {
        ...
        var options = $(
          ".starter_field[data-starter-field='" + d + "'] select option"
        );
        var curr_selected_mp_version;
        var valuetoSelect;
        if (EEVersionValue !== "None") {
          curr_selected_mp_version = valuetoSelect =
            dependencies[d][dependencies[d].length - 1];
        }
        var prev_selected_mp_version = options.filter(":selected").text();
        ...
        for (var i = 0; i < options.length; i++) {
          ...
          if (value === valuetoSelect) {
            ...
            if (
              prev_selected_mp_version !== "None" &&
              EEVersionValue !== "None" &&
              prev_selected_mp_version !== curr_selected_mp_version
            ) {
              var message = $(
                "<p>" +
                  starter_info[d].name +
    ...
    var keys;
    var EEVersion;
    var mpVersionValue = $(".starter_field[data-starter-field='m'] select")
      .find(":selected")
      .text();
    for (var starter_key in starter_dependencies) {
    ...
    var options = $(".starter_field[data-starter-field='e'] select option");
    if (!valid) {
      var prev_selected_ee_version = options.filter(":selected").text();
      for (var i = 0; i < options.length; i++) {
      ...
      if (
        mpVersionValue !== "None" &&
        prev_selected_ee_version !== "None" &&
        EEVersion !== prev_selected_ee_version
      ) {
        var message = $(
          "<p>" +
            starter_info["e"].name +
...
function displayMessage(message) {
  ...
  $("#starter_warnings").append(message).append(close_icon);
}
    ...
    .done(function (data) {
      for (var starter_field in starter_info) {
        ...
        switch (starter_field) {
          ...
          case "g": // Base Package
            $(
              ".starter_field[data-starter-field='" + starter_field + "'] input"
            ).val(default_value);
            break;
          default:
            break;
  ...
  function setFocusOnModalPopupElement() {
    $("#generate-project-modal").on("shown.bs.modal", function () {
      ...
      var focusPrevious = function () {
        currentIndex--;
        if (currentIndex < 0) currentIndex = numElements - 1;
        focus();
        return false;
      };
      ...
      var focusNext = function () {
        currentIndex++;
        if (currentIndex >= numElements) currentIndex = 0;
        focus();
        return false;
      };
    ...
    if (data.latest_releases) {
      ...
      if (latest_releases.tools) {
        if (latest_releases.tools.version) {
          $("#eclipse_developer_tools_download_link_version_text").text(
            latest_releases.tools.version
          );
        }
